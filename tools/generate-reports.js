...
async function buildDbReport() {
  ...
  const blockers = [
    ...
    {
      sev: 'HIGH', api: 'School Fees — POST /school-fees/apply (op 39)',
      obj: 'APPS.XXHMC_SND_SCHOOL_FEE_PR',
      err: 'ORA-01403: no data found @ line 197 (consistent)\nORA-00027: cannot kill current session @ line 114 (intermittent)',
      req: 'p_user_name=AIBRAHIM39, p_academic_year=2025-2026, p_acd_st_dt=20250901, p_acd_end_dt=20260630,\np_child_name=Jerome Amir Sami Samir Ibrahim (real row from CHILD_DETS_VIEW),\np_child_date_birth=20100923, p_passport_number=A38697134, p_rp_number=31081804108,\np_school_name=Al Arqam Academy (op 37 LOV), p_educational_stage=Primary (op 40 LOV),\np_request_type=Cash (op 53 LOV), p_term=Term1 (op 38 LOV), p_amount=1000 (+receipt/spouse/attachment variants)',
      reply: 'ORA-01403: no data found — ORA-06512: at "APPS.XXHMC_SND_SCHOOL_FEE_PR", line 197',
      repro: '8 payload variants tried (school by NAME and by CODE, spec sample values, with/without attachment, spouse Yes/No, academic years 2025-2026 & 2026-2027) — ALL fail at line 197. 100% of values sourced from the system\'s own LOVs + children view.',
      need: '1) Inspect the SELECT INTO at line 197 — which reference table is it reading and what row is missing for this user/child?\n2) Seed the missing reference data on staging (or handle NO_DATA_FOUND).\n3) Review the session-kill logic at line 114 (ORA-00027 = procedure tries to kill its own pooled session).',
    },
    {
    ...
    },
    {
      ...
      req: 'WORKS: p_leave_details=\'Casual Leave|19-APR-2026|19-APR-2026\' (short composite, same as amend/cancel) → passes the lookup, then stops at the Policy-Awareness prerequisite.\nFAILS: the op 55 LOV\'s own display string \'Casual Leave|Leave Start Date : 19-APR-2026 and Leave End Date : 19-APR-2026\' → ORA-06502 buffer too small.',
      reply: 'Short format: {"p_success_flag":"N","p_error_msg":"To avail this function, please submit the Policy Awareness questionnaire."} — i.e. the procedure works with the short composite.',
      repro: 'Verified both formats back-to-back on 2026-08-24.',
      need: 'Align the op 55 return LOV display value with the format the procedure actually parses (short composite Type|start|end), OR widen the line-196 buffer to accept the LOV string. Until then we document the short composite for clients.',
    },
    {
    ...
    {
      sev: 'HIGH', api: 'SYSTEMIC — several submit procedures',
    ...
    {
      sev: 'HIGH', api: 'Dependents — POST /dependents/delete (op 31) — contact type value unknown',
      obj: 'APPS.XXHMC_SND_REMOVE_DEPENDENT_PR',
      err: 'ORA-20001: The Contact Type you have entered for this Contact Relationship does not exist. (every value tried) / ORA-01407 cannot update CONTACT_TYPE to NULL (when omitted)',
      req: 'Tried with YOUR real ids (1607679 and 329303 — a confirmed Child):\n(a) p_contact_type omitted → ORA-01407 cannot update PER_CONTACT_RELATIONSHIPS.CONTACT_TYPE to NULL\n(b) p_contact_type=\'C\' + p_relationship=\'Child\' → ORA-20001 contact type does not exist\n(c) p_contact_type=\'Child\' + p_relationship=\'Child\' → same ORA-20001',
      reply: '{"p_success_flag":"N","p_error_msg":"ORA-20001: The Contact Type you have entered for this Contact Relationship does not exist."}',
      repro: 'All 4 combinations captured on 2026-08-24 (logs available via diagnostics/oracle-logs).',
      need: 'Tell us the EXACT p_contact_type value (and source lookup) REMOVE_DEPENDENT_PR expects for these contact relationships — \'C\' and \'Child\' are both rejected for a confirmed child dependent. Also: why does the procedure UPDATE CONTACT_TYPE at all during a removal?',
    },
    {
...
const API_META = {
  ...
  'Dependents|POST /dependents/update': { status: 'BLOCKED-DB', note: 'Real ids received from DB team (329302/329303/42465/1607679) but ORA-00027 at package line 3506 REGRESSED with the real id (intermittent — DB Blocker #6 reopened). Attachment mandatory. Expected success = standard S envelope.' },
  'Dependents|POST /dependents/delete': { status: 'BLOCKED-DB', note: 'Real ids received. NEW finding: p_contact_type + p_relationship must BOTH be sent (else ORA-01407), but every contact-type value tried (C / Child) is rejected — exact expected value asked from DB team (Blocker #9). Expected success = standard S envelope.' },
  ...
  // ---- School fees
  'School Fees|POST /school-fees/apply': { status: 'BLOCKED-DB', note: 'Request format final (all values from LOVs + children view). Procedure fails internally (ORA-01403 line 197 → HTTP 404; intermittent ORA-00027 → 500) — DB Blockers #2. When fixed: standard S envelope.' },
...
async function buildBackendTasks() {
  ...
  const tasks = [
    ['HIGH', 'Deploy', 'Redeploy staging with latest main', 'Latest main carries: person_id support on GET /leave/lov/amend (view is person-scoped per DB team), composite-format Swagger examples for leave amend/cancel/return, PERSON_ID rules, corrected DTO docs. Staging Swagger must match the verified Postman collection.', '—'],
